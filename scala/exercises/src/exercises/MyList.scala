...
abstract class MyList[+A] {
  ...
  // higher-order functions. Critical concept to functional programming. Uses functions as first class values
...
object ListTest extends App {
  ...
  println(
    listOfIntegers
      .flatMap(n => Cons(n, new Cons(n + 1, Empty)))
      .toString
  )

  println(
    cloneListOfIntegers == listOfIntegers
  ) // true because we made Cons a case class
